...
3. automated markup and Axe checks with no serious WCAG 2.1 A or AA violations across all
   ASIG themes; unresolved scanner results are either eliminated or narrowly allowlisted
   with a deterministic fallback check covering the same rendered content;
...
Release-specific results live in `docs/releases/`. Automated and human evidence remain
separate so an unperformed assistive-technology check cannot accidentally appear complete.
