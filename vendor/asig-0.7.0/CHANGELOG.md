...
- Added framework-neutral component contracts: input types/defaults, native events,
  recommended adapter payloads, controlled state, loading, failure and cancellation.
- Added React/Next.js, Angular and plain JavaScript integration mappings without introducing
  framework dependencies or claiming a JavaScript component API.
- Added the existing Angel Stones favicon to the catalogue, guidance reader and examples,
  including deployments beneath a GitHub Pages project prefix.
- Refreshed wiki, package guidance and distribution instructions. Token values, CSS selectors
  and component interaction implementations are unchanged; product adoption stays separate.
