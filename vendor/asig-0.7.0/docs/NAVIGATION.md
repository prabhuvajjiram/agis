...
| --- | --- | --- | --- |
| Navigation / breadcrumbs | Items: ordered list of {id, label, href}, required; current route ID: string or null, required from router | Native link activation; router destination is href | Product router owns route, history, focus and permissions; breadcrumbs may render the current item as plain text with no event |
| Sidebar / module rail | Modules and permitted destinations: labelled IDs, required; current route: string, required; expanded module ID: string or null; query: string, default empty | Recommended module-change {moduleId: string}; query-change {query: string}; native destination link activation | Module buttons change the panel, destination links change route. Filtering never grants access or changes the current route |
| Tabs | Items: {id, label, panelId, disabled?}[], required; selectedId: string, required; activation: automatic/manual, default manual | Recommended selection-change {id: string} when activated; focus movement alone is not selection in manual mode | Adapter owns roving focus and ARIA relationships; product controls selected panel and unsaved-change policy |
| Data view switcher | Views: {id, label}[], required; selectedId: string, required; busy: boolean, default false | Recommended view-change {id: string} | Product updates URL/query and loads results; does not imply panels or a tablist |
| Pagination | page: positive integer, default 1; pageSize: positive integer, required; totalItems: non-negative integer or null for unknown; hasNext: boolean, required when total unknown; busy: boolean, default false | Recommended page-change request: {page: number, pageSize: number}; native links may encode the destination instead | Product validates boundaries, fetches, updates URL and announces results. Disable Previous on page 1 and Next when no next page |

Adapter notifications describe user intent; controlled state changes when the product accepts
the request. Use one-based page numbers consistently at the UI boundary and explicitly map
zero-based APIs. Do not invent a total when only the next cursor is known. Never announce
successful loading before the requested page actually arrives.
