...
| --- | --- | --- | --- |
| Native Select | id and label: string, required; options: list of {value: string, label: string, disabled?: boolean}, required; value: string, default empty; required and disabled: boolean, default false; error: string, default absent | Native `change`: Event, read currentTarget.value; adapter value-change request: {value: string or null} where null means explicitly cleared | Product supplies options and stores the selected ID; map an empty placeholder to null deliberately |
| Searchable Combobox / picker | Select inputs plus query: string, default empty; open and loading: boolean, default false; active option ID: string or null, default null; loadError: string, default absent | Recommended adapter requests: query-change {query: string}; value-change {value: string or null}; open-change {open: boolean, reason: trigger/escape/outside/select} | Product owns selection and remote search; adapter owns focus and active-option movement. Typing does not commit a selection |
| Multi-select adapter | Options as above; values: unique string IDs, default []; disabled: boolean, default false | Recommended value-change request: {values: string[]} with the entire next selection | Explicit product adapter; not a shipped ASIG multi-select runtime |

Callback names are recommendations, not dispatched ASIG custom events. Native select values
are strings; do not use display labels as record identity. A remote adapter must ignore stale
search responses, expose loading/failure, and preserve the selected value on failure.
Escape or outside dismissal closes without a value-change request. Selection resets the
picker's temporary query; a clear action emits null only when clearing is allowed.
