...
| --- | --- | --- | --- |
| Alert / status | Message: string, required; tone: info/success/warning/danger, default info; title: optional; urgent: boolean, default false; dismissible: boolean, default false | None for static messages; recommended dismiss request {id: string} or action request {id: string, actionId: string} when explicit controls exist | Product supplies truthful message and urgency; adapter selects alert vs polite status without duplicate announcements |
| Toast | id and message: string, required; duration: positive milliseconds or null for persistent, default null; optional labelled action | Recommended dismiss request {id: string, reason: user/timeout}; action request {id: string, actionId: string} | Product adapter owns queue, timing and focus/hover pause; CSS has no timer. Actionable errors remain persistent |
| Loading / progress / skeleton | busy: boolean, default false; accessible status: string, required while busy; progress: 0–100 or null for indeterminate, default null | None; presentation does not emit completion | Product supplies actual progress and completion. Hide skeleton shapes from assistive technology |
| Empty / failed state | Title and explanation: string, required; optional labelled recovery action; busy: boolean, default false | Recommended recovery request {actionId: string}, such as retry or clear-filters | Product determines safe recovery and whether data is empty, filtered, restricted or unavailable |

Announcements are accessibility effects, not business events. Render success only after the
operation succeeds; a timer, closed toast or completed animation is never evidence of a save.
