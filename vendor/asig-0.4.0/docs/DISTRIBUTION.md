...
ASIG `0.4.0` uses a versioned vendored snapshot as its internal distribution model.
The current snapshot lives at `vendor/asig-0.4.0/` and contains the generated design tokens,
framework-neutral pattern CSS, guidance, package metadata, README, changelog, and a SHA-256
manifest.

Released snapshots are immutable. `vendor/asig-0.3.0/` remains available for existing
consumers; building `0.4.0` creates a new directory and must not rewrite the older snapshot.
