...
For direct browser delivery, `@angelstones/ui-patterns/bundle.css` provides the same rules as
`all.css` in one generated file without changing the existing consumer import.
...
Version `0.5.1` is the current release. It adds contrast-safe solid fallbacks underneath
gradient-backed actions and stronger control boundaries for managed-browser and accessibility
resilience. Existing classes and the `all.css` export remain compatible with `0.5.0` consumers;
the single-file `bundle.css` export is additive.
